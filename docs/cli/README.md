...
CLI envelope 保持 protocol version 1。backend control/business protocol 在本阶段为 version 3；旧 daemon 不会被新 client 接管。升级时先用旧 CLI 停止旧 daemon，再安装新 CLI/native 资源并启动 backend。
