...
~/.lorelum/
  ...
  revision-delivery/.lock                  # separate post-commit hook delivery lock
```
...
- **Cold open** normally remains lock-free. It reads manifest A, one SQLite transaction containing metadata / Active Packs / Effective Practices / sources, validates artifacts and projections against that snapshot, then reads manifest B. It returns only when A and B are byte-identical and the SQLite tuple matches. A manifest change retries; a stable mismatch with a live writer is `StoreBusyError`; a stable mismatch without one is `StoreRecoveryRequiredError`. If a journal exists, cold open first acquires the mutation lock, converges the journal, releases the lock, and restarts the read-only protocol. Recovery writes never run beside a live writer.
...
- **Operation journal & cross-medium recovery state machine.** The filesystem and SQLite have no shared transaction, so each manifest-mutating operation (`install`/`upgrade`/`uninstall`/`reindex`) writes a journal record carrying `{oldGeneration, targetGeneration, oldEffectiveRevision, targetEffectiveRevision, oldManifest, targetManifest}` before publishing the target manifest. `oldManifest` and `targetManifest` are complete, schema-validated manifest payloads, not deltas; they are the recovery preimage and target image, so rollback never reconstructs a Pack list from tuple values. Journal convergence is a write operation and therefore runs only while holding the mutation lock. Recovery compares **both** `(generation, effectiveRevision)` from the journal against SQLite's derived `(installedPacksGeneration, effectiveRevision)`: (a) SQLite == target → publish target and clear; (b) SQLite == old → restore old and clear; (c) neither → `StoreRecoveryRequiredError`. `reindex` first attempts this normal convergence when SQLite is readable. If SQLite itself cannot adjudicate, a successful rebuild from the current active manifest atomically supersedes and removes the older journals; it never returns success while leaving an incompatible journal behind.
...
- **Stale-lock recovery.** Each new lock carries PID, process start time, acquisition time, and a random owner token. Reclaimers atomically claim a per-record guard, re-read and compare the complete identity, and only then delete; two reclaimers cannot delete a newly-created holder. Process start time distinguishes PID reuse where the platform exposes it, with conservative fallback for legacy records. Mutation acquisition may claim a stale lock first, but lifecycle code must run journal recovery before any new write. Cold open only reclaims after its consistency check.
...
Result objects (`InstallResult`/`UninstallResult`/`ReindexResult`) carry: the committed `(generation, effectiveRevision)` tuple; the list of Practice ids added/changed/invalidated; any non-blocking `validatePack` warnings/infos; and a `cleanupPending: boolean` + `notificationPending?: { revision; error }` diagnostic for the oldest durable outbox row (§3.3 GC, §4 hook). Typed errors: `PracticeConflictError`, `PackNotInstalledError`, `StoreBusyError`, `StoreRecoveryRequiredError` (recovery entry: `reindex`), and `StoreCounterExhaustedError`. A mutation that committed but whose post-commit hook/notification or GC failed is **success** with a pending diagnostic — never re-reported as failure (§3.3, §4).
