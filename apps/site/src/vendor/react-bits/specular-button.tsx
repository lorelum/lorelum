/*
 * Vendored from React Bits (https://reactbits.dev) — SpecularButton
 * (TypeScript variant; logic ported 1:1 from the official JS + CSS source).
 * Source: https://reactbits.dev/components/specular-button
 * License: MIT + Commons Clause (see https://reactbits.dev/LICENSE.md).
 * See apps/site/THIRD_PARTY_NOTICE.md.
 *
 * Local addition: the `enableFx` prop (default true) lets the motion-aware
...
export interface SpecularButtonProps {
  ...
  autoAnimate?: boolean;
  disabled?: boolean;
  /** Local addition: mount the WebGL specular canvas at all. */
  enableFx?: boolean;
...
const SpecularButton = ({
  ...
  const propsRef = useRef<ResolvedFxProps>({
    radius,
    lineColor,
    baseColor,
    intensity,
  ...
  useEffect(() => {
    ...
    const resize = () => {
      // Layout size (offsetWidth/Height), NOT getBoundingClientRect(): the
      // landing wraps CTAs in scroll-scale transforms, and gBCR returns the
      // *visual* size — mid-scale it would pin the SDF to a shrunken, offset
      // ghost border. The canvas box (inset -20px) is anchored to the layout
      // box and scales together with the button, so layout size stays aligned
      // under any ancestor transform.
